...
class Quizes_List_Table extends WP_List_Table{
    ...
    public function __construct($plugin_name) {
        ...
        parent::__construct( array(
            'singular' => __( 'Quiz', 'quiz-maker' ), //singular name of the listed records
            'plural'   => __( 'Quizzes', 'quiz-maker' ), //plural name of the listed records
    ...
    public function add_or_edit_quizes(){
        ...
        if( isset($_POST["quiz_action"]) && wp_verify_nonce( sanitize_text_field( $_POST["quiz_action"] ), 'quiz_action' ) ){
            ...
            $quiz_wrong_answer_text_transform = (isset($_REQUEST['ays_quiz_wrong_answer_text_transform']) && $_REQUEST['ays_quiz_wrong_answer_text_transform'] != '') ? stripslashes( sanitize_text_field( $_REQUEST['ays_quiz_wrong_answer_text_transform'] ) ) : 'none';

            // Note text decoration
            $quiz_admin_note_text_decoration = (isset($_REQUEST['ays_quiz_admin_note_text_decoration']) && $_REQUEST['ays_quiz_admin_note_text_decoration'] != '') ? stripslashes( sanitize_text_field( $_REQUEST['ays_quiz_admin_note_text_decoration'] ) ) : 'none';
            ...
            $quiz_wrong_answers_text_decoration = (isset($_REQUEST['ays_quiz_wrong_answers_text_decoration']) && $_REQUEST['ays_quiz_wrong_answers_text_decoration'] != '') ? stripslashes( sanitize_text_field( $_REQUEST['ays_quiz_wrong_answers_text_decoration'] ) ) : 'none';
            ...
            $quiz_quest_explanation_letter_spacing = (isset($_REQUEST['ays_quiz_quest_explanation_letter_spacing']) && $_REQUEST['ays_quiz_quest_explanation_letter_spacing'] != '') ? stripslashes( absint( $_REQUEST['ays_quiz_quest_explanation_letter_spacing'] ) ) : 0;

            // Letter spacing | Right answer
            $quiz_right_answers_letter_spacing = (isset($_REQUEST['ays_quiz_right_answers_letter_spacing']) && $_REQUEST['ays_quiz_right_answers_letter_spacing'] != '') ? stripslashes( absint( $_REQUEST['ays_quiz_right_answers_letter_spacing'] ) ) : 0;
            ...
            $quiz_right_answers_font_weight = (isset($_REQUEST['ays_quiz_right_answers_font_weight']) && $_REQUEST['ays_quiz_right_answers_font_weight'] != '') ? stripslashes( sanitize_text_field( $_REQUEST['ays_quiz_right_answers_font_weight'] ) ) : 'normal';
            ...
            $quiz_timer_warning_text_color = (isset($_REQUEST['ays_quiz_timer_warning_text_color']) && $_REQUEST['ays_quiz_timer_warning_text_color'] != '') ? stripslashes( sanitize_text_field( $_REQUEST['ays_quiz_timer_warning_text_color'] ) ) : '#ff0000';
            ...
            // Note text transform size | Mobile
            $quiz_quest_explanation_mobile_text_transform = (isset($_REQUEST['ays_quiz_quest_explanation_mobile_text_transform']) && $_REQUEST['ays_quiz_quest_explanation_mobile_text_transform'] != '') ? stripslashes( sanitize_text_field( $_REQUEST['ays_quiz_quest_explanation_mobile_text_transform'] ) ) : 'none';

            // Right answer text transform size | Mobile
            $quiz_right_answer_mobile_text_transform = (isset($_REQUEST['ays_quiz_right_answer_mobile_text_transform']) && $_REQUEST['ays_quiz_right_answer_mobile_text_transform'] != '') ? stripslashes( sanitize_text_field( $_REQUEST['ays_quiz_right_answer_mobile_text_transform'] ) ) : 'none';

            // Wrong answer text transform size | Mobile
            $quiz_wrong_answer_mobile_text_transform = (isset($_REQUEST['ays_quiz_wrong_answer_mobile_text_transform']) && $_REQUEST['ays_quiz_wrong_answer_mobile_text_transform'] != '') ? stripslashes( sanitize_text_field( $_REQUEST['ays_quiz_wrong_answer_mobile_text_transform'] ) ) : 'none';
            ...
            $options = array(
                ...
                'quiz_quest_explanation_mobile_text_transform'      => $quiz_quest_explanation_mobile_text_transform,
    ...
    public function no_items() {
        echo esc_html__( 'There are no quizzes yet.', 'quiz-maker' );
    }
    ...
    function column_code_include( $item ) {
        $shortcode = htmlentities('\'[ays_quiz id="'.absint($item["id"]).'"]\'');
        return sprintf('<input type="text" onClick="this.setSelectionRange(0, this.value.length)" readonly value="<?php echo do_shortcode('.(esc_attr($shortcode)).'); ?>" style="max-width:100%%;" />', absint($item["id"]));
    }
    ...
    function column_published( $item ) {
        ...
        switch( $status ) {
            case 1:
                $status_html = '<span class="ays-publish-status"><i class="ays_fa ays_fa_check_square_o" aria-hidden="true"></i>'. esc_html__('Published','quiz-maker') . '</span>';
                break;
            case 0:
                $status_html = '<span class="ays-publish-status"><i class="ays_fa ays_fa_square_o" aria-hidden="true"></i>'. esc_html__('Unpublished','quiz-maker') . '</span>';
                break;
            default:
                $status_html = '<span class="ays-publish-status"><i class="ays_fa ays_fa_square_o" aria-hidden="true"></i>'. esc_html__('Unpublished','quiz-maker') . '</span>';
                break;
    ...
    function get_columns() {
        $columns = array(
            ...
            'quiz_image'            => __( 'Image', 'quiz-maker' ),
            'quiz_category_id'      => __( 'Category', 'quiz-maker' ),
            'shortcode'             => __( 'Shortcode', 'quiz-maker' ),
            'code_include'          => __( 'Code include', 'quiz-maker' ),
            'items_count'           => __( 'Count', 'quiz-maker' ),
            'create_date'           => __( 'Created', 'quiz-maker' ),
            'completed_count'       => __( 'Completed count', 'quiz-maker' ),
    ...
    public function quiz_notices(){
        ...
        if ( 'created' == $status )
            $updated_message = esc_html( __( 'Your quiz is successfully created.', 'quiz-maker' ) );
        elseif ( 'updated' == $status )
            $updated_message = esc_html( __( 'Your quiz is successfully saved.', 'quiz-maker' ) );
        elseif ( 'duplicated' == $status )
            $updated_message = esc_html( __( 'Your quiz is successfully duplicated.', 'quiz-maker' ) );
        elseif ( 'deleted' == $status )
            $updated_message = esc_html( __( 'Your quiz is successfully deleted.', 'quiz-maker' ) );
        elseif ( 'published' == $status )
            $updated_message = esc_html( __( 'Quiz(s) published.', 'quiz-maker' ) );
        elseif ( 'unpublished' == $status )
            $updated_message = esc_html( __( 'Quiz(s) unpublished.', 'quiz-maker' ) );
