(function ($) {
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '#ays-question-table-add tbody tr.ays_quest_row', function(e){
            var _this = $(this);
            var currentCheckbox = $(this).find(':checkbox');
            var id = _this.data('id');
            
            if (e.shiftKey && lastChecked != -1) {
                var eachRow =_this.parents('#ays-question-table-add').find('.ays_quest_row');
                var start = eachRow.index( this );
                var end = eachRow.index(lastChecked);
                var selectedQuestions = eachRow.slice(Math.min(start,end), Math.max(start,end)+ 1);
                if (_this.hasClass('selected')){ // do unselect
                    selectedQuestions.each(function(e){
                        var $this = $(this);
                        if($this.hasClass('selected')){
                            aysQuizSelectDeselectAddQuesiton($this);
                        }
                    });
                } else { // do select
                    selectedQuestions.each(function(e){
                ...
                aysQuizSelectDeselectAddQuesiton(_this);
            }

            lastChecked = _this;                    
        });

        function aysQuizSelectDeselectAddQuesiton(_this) {
            var id = _this.data('id');
            var index = $.inArray(id, window.aysQuestSelected);
            var index2 = $.inArray(id, window.aysQuestNewSelected);
            ...
            if(_this.hasClass('selected')){
                _this.find('.ays-select-single').removeClass('ays_fa_check_square_o').addClass('ays_fa_square_o');
            }else{
                _this.find('.ays-select-single').removeClass('ays_fa_square_o').addClass('ays_fa_check_square_o'); 
            }
        ...
        if(createdNewQuiz && createdNewQuiz > 1){
            ...
            var getCustomPostId = aysQuizGetCookie('ays_quiz_created_new_'+createdNewQuiz+'_post_id');
            ...
            swal({
                title: '<strong class="ays-quiz-maker-swal2-title">' + quizLangObj.greateJob + '</strong>',
                type: 'success',
                html: '<p class="ays-quiz-maker-swal2-created-row">' + quizLangObj.youQuizIsCreated + '</p><p clas="ays-quiz-maker-swal2-content-text-row">' + quizLangObj.youCanUuseThisShortcode + '</p><input type="text" id="ays-quiz-create-new" onClick="this.setSelectionRange(0, this.value.length)" readonly value="[ays_quiz id=\'' + createdNewQuiz + '\']" />' + htmlContent,
                ...
                cancelButtonText: '<a href="'+ link +'" target="_blank">'+ quizLangObj.preivewQuiz + ' ' + previewButtonSvgIcon +'</a>',
                cancelButtonAriaLabel: quizLangObj.thumbsUpGreat,
            });
            aysQuizDeleteCookie('ays_quiz_created_new');
            aysQuizDeleteCookie('ays_quiz_created_new_'+createdNewQuiz+'_post_id');
        }
    ...
    function openQuizMediaUploader(e, element) {
        ...
        let aysUploader = wp.media({
            ...
            if(element.hasClass('add-quiz-bg-image')){
                ...
                element.text('Edit Image');
                element.parents('.ays-field').find('.ays-quiz-image-container').fadeIn();
                element.parents('.ays-field').find('img#ays-quiz-img').attr('src', imgurl);
...
function selectAndCopyElementContents(el) {
    if (window.getSelection && document.createRange) {
        var _this = jQuery(document).find('.ays-quiz-copy-element-box');
